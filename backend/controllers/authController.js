...
exports.sendOTP = async (req, res) => {
  try {
    ...
    const accessEntry = await Access.findOne({ phoneNumber });
    let effectiveRole = 'passenger';
    if (accessEntry) {
      if (accessEntry.role === 'Admin') effectiveRole = 'admin';
      else if (accessEntry.role === 'Conductor') effectiveRole = 'conductor';
    }

    const effectiveName = accessEntry?.name || `${effectiveRole.charAt(0).toUpperCase()}${effectiveRole.slice(1)} User`;
    const effectiveConductorId = effectiveRole === 'conductor' ? accessEntry?.conductorId : undefined;
    ...
    // ✅ always return OTP in response during development
    res.json({ 
      message: 'OTP sent successfully',
...
exports.verifyOTP = async (req, res) => {
  try {
    ...
    // include assignedRouteNumber only if conductor
    const accessEntry = await Access.findOne({ phoneNumber }).populate('assignedRoute', 'routeNumber');
    const userData = {
      id: user._id,
      phoneNumber: user.phoneNumber,
      role: user.role,
