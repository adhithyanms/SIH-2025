...
const BusList: React.FC<BusListProps> = ({ onSelectBus }) => {
  ...
  if (loading) {
    return <div className="text-center text-gray-600">Loading buses...</div>;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4">
        {buses.map((bus) => (
          <div
            key={bus.id}
            className={`bg-white border-2 rounded-2xl p-6 cursor-pointer transition-all hover:shadow-lg ${getCrowdBgColor(
              bus.crowdLevel
            )}`}
            onClick={() => onSelectBus(bus.id)}
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="text-right">
                <div
                  className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium text-white ${getCrowdColor(
                    bus.crowdLevel
                  )}`}
                >
                  <div className="w-2 h-2 bg-white rounded-full mr-2"></div>
