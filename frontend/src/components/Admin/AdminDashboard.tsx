...
// import { BarChart3, Bus, Users, Route, AlertTriangle, Settings } from 'lucide-react';
...
//   const [activeTab, setActiveTab] = useState<'overview' | 'buses' | 'routes' | 'users' | 'analytics'>('overview');
//   const { buses } = useBus();
...
const AdminDashboard: React.FC = () => {
  const [activeTab, setActiveTab] = useState<
    "overview" | "buses" | "routes" | "users" | "analytics" | "access"
  >("overview");
  const { buses } = useBus();
  ...
  const totalPassengers = buses.reduce(
    (sum, bus) => sum + bus.passengerCount,
    0
  );
  const alerts = buses.filter((bus) => bus.crowdLevel === "high").length;
